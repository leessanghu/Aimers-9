"""v24 = v15 피처 100% 동일 + 실측으로 검증된 두 개의 독립 양수 신호를 결합.
  (1) SEED 42 -> 10 (v21 실측 +1.64)
  (2) team_te shuffled_kfold -> expanding (v23 실측 -1.02, 4seed 로컬 평균은 +23.2였지만
      이번 1회 실측은 미세하게 마이너스 — 다른 seed와 조합하면 다를 수 있어 재검증)
둘 다 단독으로는 로컬/실측이 엇갈렸지만, 서로 독립적인 변경이라 결합 효과를 직접 확인한다.
...
def main():
    ...
    for tag, w_hgb, w_cat in [("v24", 0.5, 0.5)]:
