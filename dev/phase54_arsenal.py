"""구종 레퍼토리 엔트로피 검증 — baseline = v15 구성(hidden_denom 없음).
...
leakage 걱정이 전혀 없다(crosses.py와 같은 철학).
...
from arsenal_entropy import ARSENAL_COLS, transform_arsenal
...
print("구종 레퍼토리 엔트로피 계산...", flush=True)
darsenal = transform_arsenal(df)
print(f"  ({time.time()-t0:.0f}s)", flush=True)
for c in ARSENAL_COLS:
    print(f"  {c:18s} SD={darsenal[c].std():.5f}  mean={darsenal[c].mean():.5f}", flush=True)
...
print("\n=== 구종 엔트로피 개별 잔차가치 ===", flush=True)
tot = 0.0
for c in ARSENAL_COLS:
    z = darsenal.loc[va, c].to_numpy(np.float64)
    if z.std() < 1e-12:
        continue
...
print("\n=== 직접 폴드 재확인 (v15구성 + 구종엔트로피 2개, HGB+cat단일) ===", flush=True)
Xtr2 = pd.concat([Xtr, darsenal.loc[tr].reset_index(drop=True)], axis=1)
Xva2 = pd.concat([Xva, darsenal.loc[va].reset_index(drop=True)], axis=1)
...
print(f"  baseline(v15)={base:.1f}  +구종엔트로피2={s2:.1f}  delta={s2-base:+.1f}", flush=True)
