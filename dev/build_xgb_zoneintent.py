"""실험: zone_intent(판정축의 거친 2분할, 성공행 52.3%만 관측) 5클래스를 XGB로.
mc6와 정보상 중복이라 이론상 기대 낮음(LGBM-mc6와 같은 함정 우려) - 그래도 저비용 검증.

fold A만, v122 기준 직교화(vs d_mc6, d_xu) + 순열대조군 z.
"""
import sys, time
...
np.save('dev/cache_xgbzi_A.npy', p)
...
ctrl = []
for sd in range(20):
    rng = np.random.RandomState(8000 + sd)
