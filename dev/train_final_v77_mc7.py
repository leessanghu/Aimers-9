...
# 7-class: nd&ball을 strikes_before(0/1/2)로 3분할.
# 근거: nd&ball 안에서 strikes별 성공률 62.17/60.05/53.56% (8.6%p 격차)이고,
# nd&ball의 class기여분산이 0.0012로 5개 중 유독 낮아(=정보 부족 구간) 세분화 여지가 크다.
NCLS = 7
strikes = df["strikes_before"].to_numpy(np.int64)
...
cls[ndb & (strikes == 1)] = 3
cls[ndb & (strikes >= 2)] = 4
cls[nd & (lab_ball < 0.5) & (lab_strike > 0.5)] = 5
cls[nd & (lab_ball < 0.5) & (lab_strike < 0.5)] = 6
fit_mask = cls >= 0
succ_by_cls = np.array([y[fit_mask & (cls == c)].mean() for c in range(NCLS)])
...
out = os.path.join(OUT_DIR, "model_artifacts_v77.pkl")
