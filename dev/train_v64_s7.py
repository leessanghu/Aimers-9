"""v64 = v50 + future50_multi축(y + 향후50투구 성공률 + 향후50투구 1-middle율).
v49 formcast(단일 미래헤드, 실측+3.41) 확장판. idea54 fold A 로컬Δ=-1.63(음수)였으나
사용자 지시로 실측 직행. WINDOW=50, K=10 축소. train 전용 미래정보(Rule §4 준수,
추론시 head0만 사용).
"""
import os
import sys
...
log("투구단위 middle 라벨 복원 + 향후50투구 슬라이딩 head 구성...")
...
c_ord = c_mid[order]
d_ord = np.empty(len(df)); d_ord[:-1] = c_ord[1:] - c_ord[:-1]; d_ord[-1] = np.nan
d_ord[~same_next[order]] = np.nan
lab_middle = np.empty(len(df)); lab_middle[order] = d_ord

WINDOW = 50
K_F = 10.0
g = float(y.mean()); g_mid = float(np.nanmean(lab_middle))
sub = pd.DataFrame({"pid": pid, "row_num": df["row_num"].to_numpy(), "y": y, "mid": lab_middle})
...
h_fut_succ = ((fys + K_F * g) / (fyc + K_F))
h_fut_notmid = 1.0 - ((fms + K_F * g_mid) / (fmc + K_F))
head_fut_succ = pd.Series(h_fut_succ.to_numpy(), index=sub.index).reindex(range(len(df))).to_numpy(np.float64)
head_fut_notmid = pd.Series(h_fut_notmid.to_numpy(), index=sub.index).reindex(range(len(df))).to_numpy(np.float64)
log(f"  future50 커버리지 {np.isfinite(head_fut_succ).mean()*100:.1f}%")
Ymat = np.column_stack([y.astype(np.float64), head_fut_succ, head_fut_notmid])

log("future50_multi축 공유트리 CatBoost 학습 (전체데이터)...")
...
CAT_PARAMS = dict(iterations=1500, learning_rate=0.03, depth=6, l2_leaf_reg=5.0, verbose=0,
                  random_seed=7, loss_function="MultiRMSEWithMissingValues", early_stopping_rounds=50)
ts = time.time()
future50_model = CatBoostRegressor(**CAT_PARAMS)
future50_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                     eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={future50_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(future50_model)

common = dict(v50)
common["future50_model"] = future50_model
common["future50_weight"] = FUTURE50_WEIGHT
...
common["base_weight"] = 1.0 - existing - FUTURE50_WEIGHT
log(f"weights: base={common['base_weight']:.3f} future50={FUTURE50_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v64s7.pkl")
