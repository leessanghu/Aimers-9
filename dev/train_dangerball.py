"""danger_ball 프로덕션 모델 — v62 cond_ball의 여집합.
idea55 검증: dangerous(middle or reverse) 행에서만 1-ball head, 나머지 NaN(coverage 34.44%).
2시드 fold A: solo 847.32/837.94, 시드폭 9.26 (v62 cond_ball 21.43보다 안정).
v66에 '추가'는 포화로 음수(-2.93)지만 'count_resid 대체'는 +0.697, 'future50 대체'는 +1.276.
-> 대체용으로 생존. 이 스크립트는 모델만 학습하고, 블렌딩은 별도 병합에서 처리.
"""
import os
import sys
import time
...
head_dangerball = np.where(dang, 1.0 - lab_ball, np.nan)
log(f"  danger_ball coverage={np.isfinite(head_dangerball).mean()*100:.2f}%")
Ymat = np.column_stack([y.astype(np.float64), head_dangerball])

log("danger_ball축 공유트리 CatBoost 학습 (전체데이터)...")
...
ts = time.time()
dangerball_model = CatBoostRegressor(**CAT_PARAMS)
dangerball_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                     eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={dangerball_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(dangerball_model)

common = dict(v50)
common["dangerball_model"] = dangerball_model
common["dangerball_weight"] = DANGERBALL_WEIGHT
...
common["base_weight"] = 1.0 - existing - DANGERBALL_WEIGHT
log(f"weights: base={common['base_weight']:.3f} condball={DANGERBALL_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_dangerball.pkl")
