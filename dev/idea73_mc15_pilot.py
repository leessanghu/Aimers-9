...
cls5 = np.load("cls5_labels.npy")
pt = np.load("pitchtype_labels.npy")
...
P = m.predict_proba(X.loc[va])          # (n_va, NCLS)
np.save(f"{CD}/A_proba15.npy", P)
...
for c in range(NCLS):
    print(f"  class{c:2d}: 예측평균={P[:,c].mean():.4f} 실제={np.mean(cls[va]==c):.4f}")
