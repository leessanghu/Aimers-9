"""LGBM-mc6: mc6와 동일한 6클래스 순수분할 타겟, 알고리즘만 LightGBM.

가설: 지금까지 기각된 이질알고리즘(xgb/lgbm_rawid 등)은 전부 binary-y 모델.
     가장 강한 축(mc6 타겟재정의) 위에 알고리즘 다양성을 얹는 건 미탐색.
     CatBoost-mc6와 같은 타겟이지만 분기방식이 달라 다른 투영을 잡을 수 있음.

검증: fold A만(fold C는 전역편향으로 정보없음 - 오늘 확정), v122 기준
     직교화(vs d_mc6, d_xu) + 순열대조군 z검정. 통과시에만 프로덕션.
"""
import sys, time
...
np.save('dev/cache_lgbmmc6_A.npy', p)
log(f'fold A 학습완료 best_iter={m.best_iteration_} ({time.time()-ts:.0f}s)')
...
v117b = rest * blend8 + S_MC6 * p_mc6 + S_STRK * p_strk
blend = np.clip(v117b * (1 - S_XU) + S_XU * p_xu, 0, 1)
resid = yv - blend
...
print(f'\nLGBM-mc6 단독 BSS = {sc(p):.2f}   (참고: CatBoost-mc6 단독 = {sc(p_mc6):.2f})')
print(f'예측상관(LGBM-mc6 vs Cat-mc6) = {np.corrcoef(p, p_mc6)[0,1]:.4f}')

d = p - blend; d -= d.mean()
V = float(np.mean(d ** 2)); A = float(np.mean(d * (blend - yv)))
rho0 = -A / np.sqrt(V * E_r2)
print(f'원본:    rho={rho0:+.5f}  이득={K*A**2/V:+.2f}  s*={-A/V:+.4f}  z(SE)={abs(rho0)/SE:.1f}')
...
d_mc6 = p_mc6 - blend
d_xu = p_xu - blend
d_perp = orth(d, [d_mc6, d_xu])
Vp = float(np.mean(d_perp ** 2)); Ap = float(np.mean(d_perp * (blend - yv)))
rho_p = -Ap / np.sqrt(Vp * E_r2) if Vp > 1e-16 else 0.0
print(f'직교화후: rho={rho_p:+.5f}  이득={K*Ap**2/Vp if Vp>1e-16 else 0:+.2f}  '
      f's*={-Ap/Vp if Vp>1e-16 else 0:+.4f}  z(SE)={abs(rho_p)/SE:.1f}')

ctrl = []
for sd in range(20):
    rng = np.random.RandomState(3000 + sd)
    dc = orth(rng.permutation(d), [d_mc6, d_xu])
    Vc = float(np.mean(dc ** 2))
    if Vc < 1e-16:
        continue
    Ac = float(np.mean(dc * (blend - yv)))
    ctrl.append(-Ac / np.sqrt(Vc * E_r2))
ctrl = np.array(ctrl)
z = (abs(rho_p) - np.abs(ctrl).mean()) / (np.abs(ctrl).std(ddof=1) + 1e-18)
print(f'대조군: 평균={ctrl.mean():+.5f} SD={ctrl.std(ddof=1):.5f} 최대|={np.abs(ctrl).max():.5f}')
print(f'실제 vs 대조군 z = {z:.1f}  ->  {"통과(진짜신호)" if z > 3 else ("경계" if z > 1.5 else "허수")}')
log('완료')
