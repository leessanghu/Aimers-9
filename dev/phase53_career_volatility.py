"""커리어 시즌간 변동성(std/min/max) 검증 — baseline = v15 구성.

AMEX/Home Credit류 "entity 과거 관측치를 std/min/max로 압축" 패턴. 지금까지 우리는
'평균 제구력'만 썼지, 이 투수가 시즌마다 얼마나 들쭉날쭉한지(변동성)는 안 봤다.
...
from career_volatility import VOL_COLS, K_VOL, build_volatility_table, transform_volatility
...
print("커리어 변동성 테이블 구축...", flush=True)
vol_tbl = build_volatility_table(se)
dvol = transform_volatility(df, vol_tbl, sr, k=K_VOL)
print(f"  vol_n_seasons>0 비율={(dvol['vol_n_seasons']>0).mean():.3f}  ({time.time()-t0:.0f}s)", flush=True)
for c in ["vol_std", "vol_min", "vol_max", "vol_range"]:
    print(f"  {c:14s} SD={dvol[c].std():.5f}  mean={dvol[c].mean():.5f}", flush=True)
...
print("\n=== 커리어 변동성 개별 잔차가치 ===", flush=True)
tot = 0.0
for c in VOL_COLS:
    z = dvol.loc[va, c].to_numpy(np.float64)
    if z.std() < 1e-12:
        continue
...
print("\n=== 직접 폴드 재확인 (v15구성 + 변동성 5개, HGB+cat단일) ===", flush=True)
Xtr2 = pd.concat([Xtr, dvol.loc[tr].reset_index(drop=True)], axis=1)
Xva2 = pd.concat([Xva, dvol.loc[va].reset_index(drop=True)], axis=1)
...
print(f"  baseline(v15)={base:.1f}  +변동성5={s2:.1f}  delta={s2-base:+.1f}", flush=True)

m19 = df["season"] == 2019
print(f"\n  2019행 vol_n_seasons 최대={dvol.loc[m19,'vol_n_seasons'].max():.2e} (0이어야 정상)", flush=True)
