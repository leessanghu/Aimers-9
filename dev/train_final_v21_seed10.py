"""v21_seed10 = v15(976.099 실증, 최고점)와 피처/코드 100% 동일, SEED만 42 -> 10.

순수 진단용 실험: v15 이후 시도한 5개(v17a -10.1, v17b -3, v18 -2.44, v19 -3.3, v20 -3.83)가
전부 v15보다 나빴고 972~974점 사이에 좁게 뭉쳐 있었다. 이게 "우리가 세션 내내 모든 결정을
CatBoost/HGB seed=42 하나로만 검증해오면서 알게 모르게 그 시드에 맞춰진 결과"인지,
아니면 "v15 이후 피처 자체가 전부 안 좋았던 것"인지 구분이 안 됐다.
그래서 피처는 전혀 안 건드리고 SEED 상수 하나만 42->10으로 바꿔서 실측 태운다.
(HGB random_state / FeatureBuilder OOF KFold seed / CatBoost random_seed 전부 이 SEED를 공유함
-> "CatBoost 시드만" 격리는 아니고 전체 파이프라인의 seed 민감도를 한 번에 보는 실험)
...
def main():
    ...
    for tag, w_hgb, w_cat in [("v21", 0.5, 0.5)]:
