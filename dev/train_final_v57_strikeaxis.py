"""v57 = v50 + strike축(판정/존) 공유트리. idea43 재검증(3시드+foldC)으로 확정한
SHAP미활용도(87위/5회, middle 122위/2회에 근접) 기반 후보.
head1=(1-lab_strike), 추론시엔 head0(y)만 사용. Rule.md §4 준수(train 누적통계
차분으로만 라벨 복원, test 행간 참조 없음).
"""
import os
import sys
import time
...
log("투구단위 strike 라벨 복원 (누적 strike_rate 차분)...")
...
c_strike = np.round(df["asof_pitcher_strike_rate"].fillna(0).to_numpy(np.float64) * n_)
c_ord = c_strike[order]
d_ord = np.empty(len(df))
d_ord[:-1] = c_ord[1:] - c_ord[:-1]
d_ord[-1] = np.nan
d_ord[~same_next[order]] = np.nan
lab_strike = np.empty(len(df))
lab_strike[order] = d_ord
valid_lab = ~np.isnan(lab_strike)
log(f"  라벨 유효행 {valid_lab.sum():,}/{len(df):,} ({valid_lab.mean()*100:.2f}%), strike율={np.nanmean(lab_strike)*100:.2f}%")

head_strike = np.where(valid_lab, 1.0 - lab_strike, np.nan)
Ymat = np.column_stack([y.astype(np.float64), head_strike])

log("strike축 공유트리 CatBoost 학습 (전체데이터)...")
...
ts = time.time()
strikeaxis_model = CatBoostRegressor(**CAT_PARAMS)
strikeaxis_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                     eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={strikeaxis_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(strikeaxis_model)

common = dict(v50)
common["strikeaxis_model"] = strikeaxis_model
common["strikeaxis_weight"] = STRIKEAXIS_WEIGHT
...
common["base_weight"] = 1.0 - existing - STRIKEAXIS_WEIGHT
log(f"weights: base={common['base_weight']:.3f} strikeaxis={STRIKEAXIS_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v57.pkl")
