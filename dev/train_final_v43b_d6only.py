"""v43b = v42 + base3의 d6만 시간순ES로 교체(가장 빠른 검증판). d8/sub는 다음 라운드.
"""
import os
import sys
import time
...
log(f"  hgbs={len(v42['hgbs'])} cats={len(v42['cats'])}")
...
X = X[v42["feature_order"]].astype(np.float64)
...
extra = dict(max_depth=6, max_leaf_nodes=31, random_state=42)
log(f"[hgb_d6] iter 그리드 스윕 {ITER_GRID}...")
...
log(f"  최적 iter={best_iter} (검증점수={best_score:.2f})")
...
strip_rng(m_full)
log(f"  전체데이터 최종학습 완료 ({time.time()-ts:.0f}s)")

new_hgbs = [m_full, v42["hgbs"][1], v42["hgbs"][2]]  # d6만 교체, d8/sub는 기존 유지
common = dict(v42)
common["hgbs"] = new_hgbs
out = os.path.join(OUT_DIR, "model_artifacts_v43b.pkl")
