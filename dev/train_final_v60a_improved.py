"""v60a = v60 + 3개 개선.
1) base HGB를 v42 원본(랜덤ES,500iter)으로 원복. refit-closure(v44이후 5세대)가
   실측 0/6 전패로 확정됐는데 아무도 원복한 적 없었음.
2) base:hurdle:multires:ordinal 비율을 v42 원본(3:4:1:2)로 복원 후 0.80으로
   비례축소(midother=0.20 확보). 지금까지는 새 aux head 추가시마다 base에서만
   깎아왔음 -> fold A/C 둘다 이 방식이 더 나음(+0.95/+6.63, local_leaderboard 규약).
3) aux head 대조군 실험(idea47)은 미해결(대조군이 fold C에서 오히려 나음) ->
   구조는 안 바꾸고 정직하게 기록만.
...
log(f"weights(비례축소): base={common['base_weight']:.3f} hurdle={common['hurdle_weight']:.3f} "
    f"multires={common['multires_weight']:.3f} ordinal={common['ordinal_weight']:.3f} midother={MIDOTHER_WEIGHT:.2f}")

out = os.path.join(OUT_DIR, "model_artifacts_v60a.pkl")
