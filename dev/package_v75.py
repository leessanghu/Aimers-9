...
ZIP_PATH = os.path.abspath(os.path.join(BASE, "..", "submit_v75_mlp.zip"))

pkl_path = os.path.join(MODEL_DIR, "model_artifacts_v75.pkl")
assert os.path.exists(pkl_path), f"모델 파일 없음: {pkl_path}"
print(f"모델 파일 확인: {pkl_path} ({os.path.getsize(pkl_path)/1e6:.1f}MB)")

other_pkls = [f for f in os.listdir(MODEL_DIR) if f.endswith(".pkl") and f != "model_artifacts_v75.pkl"]
moved = []
for f in other_pkls:
...
try:
    ...
    with zipfile.ZipFile(ZIP_PATH, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.write(pkl_path, "model/model_artifacts_v75.pkl")
