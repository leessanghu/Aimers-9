"""LightGBM + raw ID native categorical, 우리 162피처 기반. honest fold A/C 검증.
XGB raw-ID(build_xgb_rawid.py)와 똑같은 설계 원칙, 모델만 LightGBM으로 교체:
- pitcher_id/batter_id/pitcher_team_id/batter_team_id를 LightGBM category dtype으로 직접 투입
- LGBM은 leaf-wise라 과적합에 더 취약 -> num_leaves를 낮게(63), min_child_samples를 높게(30) 잡아
  raw ID 분기가 표본 충분한 곳에서만 생기도록 억제
- n_estimators=3000, lr=0.01, early_stopping은 train 내부 마지막 8%로(Rule4 안전)
...
CAT_COLS = ['pitcher_id', 'batter_id', 'pitcher_team_id_cat', 'batter_team_id_cat']
PARAMS = dict(n_estimators=3000, learning_rate=0.01, num_leaves=63, min_child_samples=30,
              subsample=0.9, subsample_freq=1, colsample_bytree=0.6, reg_alpha=0.1, reg_lambda=1.0,
              max_bin=255, random_state=42, n_jobs=-1, verbosity=-1,
              objective='binary')
...
np.save('dev/cache_lgbmrawid_A.npy', p_A)

log('=== fold C (train<=2021 -> 2022) ===')
p_C, va_C = train_eval(2021, 2022, 'C')
np.save('dev/cache_lgbmrawid_C.npy', p_C)
...
print(f'  lgbm_rawid  H1->H2={g[0]:+7.2f}  H2->H1={g[1]:+7.2f}  평균={np.mean(g):+7.2f}')
log('완료')
