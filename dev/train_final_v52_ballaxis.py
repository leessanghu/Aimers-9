"""v52 = v47 + ball축(판정/존 축) 공유트리.
idea32 스크리닝: fold A +0.43(시드폭 8.35), 단독 848.45 -- reverse(-0.64)/strike(-0.04)
보다 우수. aux head 편향규칙(+6.15) 적용시 보정추정 +6.58.
ball은 커맨드축(success/reverse/middle)과 다른 판정축이라 v50 middle과 정보중복이
적을 것으로 기대. head1=(1-lab_ball), 추론시엔 head0(y)만 사용.
...
log("투구단위 ball 라벨 복원 (누적 ball_rate 차분)...")
...
c_mid = np.round(df["asof_pitcher_ball_rate"].fillna(0).to_numpy(np.float64) * n_)
c_ord = c_mid[order]
d_ord = np.empty(len(df))
d_ord[:-1] = c_ord[1:] - c_ord[:-1]
d_ord[-1] = np.nan
d_ord[~same_next[order]] = np.nan
lab_ball = np.empty(len(df))
lab_ball[order] = d_ord
valid_lab = ~np.isnan(lab_ball)
log(f"  라벨 유효행 {valid_lab.sum():,}/{len(df):,} ({valid_lab.mean()*100:.2f}%), ball율={np.nanmean(lab_ball)*100:.2f}%")

# head1 = (1 - middle). 복원 불가행은 NaN -> MultiRMSEWithMissingValues가 해당 head만 제외
head_ball = np.where(valid_lab, 1.0 - lab_ball, np.nan)

Ymat = np.column_stack([y.astype(np.float64), head_ball])

log("ball축 공유트리 CatBoost 학습 (전체데이터)...")
...
ts = time.time()
ballaxis_model = CatBoostRegressor(**CAT_PARAMS)
ballaxis_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                   eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={ballaxis_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(ballaxis_model)

common = dict(v47)
common["ballaxis_model"] = ballaxis_model
common["ballaxis_weight"] = BALLAXIS_WEIGHT
...
common["base_weight"] = 1.0 - existing - BALLAXIS_WEIGHT
log(f"weights: base={common['base_weight']:.3f} ballaxis={BALLAXIS_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v52.pkl")
