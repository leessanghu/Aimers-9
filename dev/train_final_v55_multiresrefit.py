"""v55 = v50 + multires(투수-시즌/투수x손 LOO 공유트리) refit-closure.
v40 이후 tr_i,es_i=0.92 split 그대로(마지막 8% 트리분할 미사용). ES로 best_iter
확정 후 early_stopping 끄고 전체데이터로 고정반복 재학습. v50 base(cats=ES 원본)
위에 독립적으로 얹어 multires refit 단독효과를 격리 측정할 수 있게 함.
"""
import os
import sys
import time
...
log(f"  multires_weight={v50.get('multires_weight')}")
...
m_es.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w_rec[tr_i], eval_set=(X.iloc[es_i], Ymat[es_i]))
...
multires_model = CatBoostRegressor(**params_fixed)
multires_model.fit(X, Ymat, sample_weight=w_rec)
...
out = os.path.join(OUT_DIR, "model_artifacts_v55.pkl")
