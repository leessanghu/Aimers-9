"""v46 = v45 + base3의 HGB 3개(d6/d8/sub)를 classifier(logloss)->regressor(squared_error)로
교체. idea25 파일럿에서 3폴드 전부 양수(+1.29/+14.93/+15.01) 확인된 검증된 변경.
Hurdle의 HGB는 classifier 그대로 유지(cat_reg가 오히려 더 안 좋았던 것과 별개로,
hurdle 쪽 regressor는 검증 안 했으므로 이번엔 안 건드림).
"""
import os
import sys
import time
...
log("v45 아티팩트 로드...")
v45 = joblib.load(os.path.join(OUT_DIR, "model_artifacts_v45.pkl"))
log(f"  hgbs={len(v45['hgbs'])} cats={len(v45['cats'])} core={len(v45['core_fail_models'])}")
...
X = X[v45["feature_order"]].astype(np.float64)
...
HGB_CONFIGS = [
    ("hgb_d6_reg", dict(max_depth=6, max_leaf_nodes=31, random_state=42)),
    ("hgb_d8_reg", dict(max_depth=8, max_leaf_nodes=15, random_state=2024)),
    ("hgb_sub_reg", dict(max_depth=6, max_leaf_nodes=31, max_features=0.6, random_state=123)),
]
...
for name, extra in HGB_CONFIGS:
    ...
    for it in ITER_GRID:
        ts = time.time()
        params = dict(max_iter=it, learning_rate=0.03, l2_regularization=5.0,
                      loss="squared_error", early_stopping=False)
        params.update(extra)
        m = HistGradientBoostingRegressor(**params).fit(X.iloc[tr_i], y[tr_i], sample_weight=w[tr_i])
        p_es = np.clip(m.predict(X.iloc[es_i]), 0, 1)
        yv_es = y[es_i]
    ...
    ts = time.time()
    params_full = dict(max_iter=best_iter, learning_rate=0.03, l2_regularization=5.0,
                       loss="squared_error", early_stopping=False)
    params_full.update(extra)
    m_full = HistGradientBoostingRegressor(**params_full).fit(X, y, sample_weight=w)
    strip_rng(m_full)
    new_hgbs.append(m_full)
    log(f"  [{name}] 전체데이터 최종학습 완료 ({time.time()-ts:.0f}s)")

common = dict(v45)
common["hgbs"] = new_hgbs
out = os.path.join(OUT_DIR, "model_artifacts_v46.pkl")
