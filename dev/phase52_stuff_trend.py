"""시즌 내 구위 추세(최근 한 달 vs 그 이전 시즌누적, 구속/스핀) 검증 — baseline = v15 구성(hidden_denom 없음).

가설: 최근 구위(구속/스핀)가 시즌 초반 대비 떨어진 투수는 피로/부상으로 제구도 흔들릴 수 있다.
지금까지 Trackman은 항상 '누적 평균'만 썼으니 이건 '최근 변화율'이라는 다른 축의 새 시도.
...
from stuff_trend import STUFF_TREND_COLS, K_TREND, build_stuff_month_table, transform_stuff_trend
...
print("구위 추세 테이블 구축 (trackman_history 스캔)...", flush=True)
stuff_tbl = build_stuff_month_table()
dstuff = transform_stuff_trend(df, stuff_tbl, k=K_TREND)
print(f"  trend_n_recent>0 비율={(dstuff['trend_n_recent']>0).mean():.3f}  ({time.time()-t0:.0f}s)", flush=True)
for c in ["trend_speed", "trend_spin"]:
    valid = dstuff.loc[dstuff["trend_n_recent"] > 0, c]
    print(f"  {c:14s} SD={dstuff[c].std():.5f}  mean(있는행)={valid.mean():.5f}", flush=True)
...
print("\n=== 구위 추세 개별 잔차가치 ===", flush=True)
tot = 0.0
for c in STUFF_TREND_COLS:
    z = dstuff.loc[va, c].to_numpy(np.float64)
    if z.std() < 1e-12:
        continue
...
print("\n=== 직접 폴드 재확인 (v15구성 + 구위추세 3개, HGB+cat단일) ===", flush=True)
Xtr2 = pd.concat([Xtr, dstuff.loc[tr].reset_index(drop=True)], axis=1)
Xva2 = pd.concat([Xva, dstuff.loc[va].reset_index(drop=True)], axis=1)
...
print(f"  baseline(v15)={base:.1f}  +구위추세3={s2:.1f}  delta={s2-base:+.1f}", flush=True)

m_early = df["game_month"] <= 2
print(f"\n  month<=2행 |trend_speed| 최대={dstuff.loc[m_early,'trend_speed'].abs().max():.2e}"
      f"  |trend_spin| 최대={dstuff.loc[m_early,'trend_spin'].abs().max():.2e} (둘 다 0이어야 정상, early baseline 없음)", flush=True)
