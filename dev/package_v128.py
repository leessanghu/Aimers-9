...
ZIP_PATH = os.path.abspath(os.path.join(BASE, "..", "submit_v128_combined_nnraw.zip"))
...
pkl_path = os.path.join(MODEL_DIR, "model_artifacts_v128.pkl")
...
to_move = [f for f in os.listdir(MODEL_DIR)
           if (f.endswith(".pkl") and f != "model_artifacts_v128.pkl") or f in CODEX_LEFTOVERS]
moved = []
try:
    for f in to_move:
    ...
    assert active == ["model_artifacts_v128.pkl"], f"isolation failed: {active}"
    ...
    with zipfile.ZipFile(ZIP_PATH, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.write(pkl_path, "model/model_artifacts_v128.pkl")
