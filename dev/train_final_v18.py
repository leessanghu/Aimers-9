"""v18 = v15(976.099 실증, 최고점) + CatBoost 3시드 평균 (단일 변수 변경).

phase46(2024폴드): hgb.5+cat단일.5(v15와 동일 구성)=812.9 -> hgb.5+cat3시드.5=815.2 (+2.3)
phase47: Gated MoE 전제 기각(트리가 이미 n에 따라 gate 역할 수행 중, 잔차상관 전부 <0.02)
가중치(0.5/0.5)는 그대로 둔다 — v14a/v14b 실측에서 가중치 이동 효과가 +0.05로 사실상 없었음.
시드평균은 편향 불변/분산만 감소라 이론적으로 손해 불가능한 유일한 카드.
...
def main():
    ...
    print(f"\n교차항 {C.shape[1]}개 + 작년 {X_ly.shape[1]}개 + hidden_den {X_den.shape[1]}개 -> 최종 피처 수={X.shape[1]} (v18 91 -> 98)", flush=True)
    ...
    for tag, w_hgb, w_cat in [("v18", 0.5, 0.5)]:
