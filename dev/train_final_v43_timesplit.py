"""v43 = v42(base+hurdle+multires+ordinal) + base3의 HGB 3변종(d6/d8/sub)만 시간순ES로 교체.
시간 압박: 폴드검증 생략(d6로 이미 3폴드 일관검증됨), iter후보 3개(150/200/300)로 프로덕션 직행.
Hurdle의 HGB 4개(core/succ_nc)는 이번엔 안 건드림(다음 라운드).
"""
import os
import sys
import time
...
out = os.path.join(OUT_DIR, "model_artifacts_v43.pkl")
