"""v11 = v10(950.813 실증) + 교차항 14개 = 80피처.

변수는 교차항 하나만. 칼만/CatBoost/배합 전부 v10 그대로.
phase25 2024폴드: 826.7 -> 848.4 (+21.7). CatBoost가 826.5->855.2로 크게 개선(+28.7),
HGB는 +1.2로 거의 무변화 -> 얕은 트리가 스스로 못 만들던 곱/비율을 명시적으로 준 효과.
...
def main():
    ...
    print(f"\n교차항 {C.shape[1]}개 추가 -> 최종 피처 수={X.shape[1]}", flush=True)
    ...
    out = os.path.join(OUT_DIR, "model_artifacts_v11.pkl")
