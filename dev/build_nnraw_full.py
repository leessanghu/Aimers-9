"""nn_raw 프로덕션: 전체데이터(2019-2024) 2시드 학습. fold A z=3.7 통과.
추론은 numpy forward(torch state_dict만 export, script.py에서 순수 numpy로 재구현).
"""
...
def train_one(seed):
    ...
    mu = np.nanmean(X_raw[idx_fit], axis=0); sd = np.nanstd(X_raw[idx_fit], axis=0) + 1e-9
    def zx(rows):
        z = (X_raw[rows] - mu) / sd
    ...
    for ep in range(EPOCHS):
        ...
        if brier < best[0] - 1e-6:
            best = (brier, {k: v.detach().clone() for k, v in net.state_dict().items()}, ep); bad = 0
        else:
            bad += 1
            if bad >= patience:
                log(f'[prod/seed{seed}] 얼리스탑 (best ep{best[2]+1}, brier={best[0]:.6f})'); break
...
joblib.dump(dict(models=prod, feat_order=CONTEXT_FEATS, arch='nnraw_ctx52_emb8-8-4-4'),
            'dev/nnraw_production.pkl')
log(f'저장 완료 ({len(prod)}시드): dev/nnraw_production.pkl')
