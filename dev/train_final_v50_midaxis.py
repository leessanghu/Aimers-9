"""v50 = v47 + middle축 공유트리 (SHAP vs partial_gain 대조에서 middle 계열 피처가
일관되게 저평가 패턴을 보여 독립 auxiliary head로 승격. fold A +1.08/fold C +7.68,
시드폭 1.54/10.92로 안정. head1=(1-lab_middle), 추론시엔 head0(y)만 사용.
...
log(f"  라벨 유효행 {valid_lab.sum():,}/{len(df):,} ({valid_lab.mean()*100:.2f}%), middle율={np.nanmean(lab_middle)*100:.2f}%")
...
log("middle축 공유트리 CatBoost 학습 (전체데이터)...")
...
ts = time.time()
midaxis_model = CatBoostRegressor(**CAT_PARAMS)
midaxis_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                   eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={midaxis_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(midaxis_model)

common = dict(v47)
common["midaxis_model"] = midaxis_model
common["midaxis_weight"] = MIDAXIS_WEIGHT
...
common["base_weight"] = 1.0 - existing - MIDAXIS_WEIGHT
log(f"weights: base={common['base_weight']:.3f} midaxis={MIDAXIS_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v50.pkl")
