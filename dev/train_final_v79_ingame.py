...
# 11-class = (middle, reverse: 구종 무관 항상 성공률 0%라 안 쪼갬) + (nd&ball/nd&strike/nd&기타
# x 3구종 = 9). 15-class 대비 정보손실 0(middle/reverse는 구종별로도 전부 0.000%였음이
# 이미 확인됨) -- CatBoost MultiClass는 leaf당 KxK 뉴턴시스템을 풀어 비용이 K^2~K^3로
# 늘어나므로, 15->11로 줄이면 정보손실 없이 계산량만 크게 줄어든다(실측 벤치: depth=6
# 기준 133분->정보손실 없는 11-class로도 여전히 느려 depth=5 병행 채택, 61분).
NCLS = 11
...
nd2 = v & (cls5 >= 2)
cls[nd2] = (cls5[nd2] - 2) * 3 + pt[nd2]   # 0~8
cls[v & (cls5 == 0)] = 9                    # middle (구종 무관)
cls[v & (cls5 == 1)] = 10                   # reverse (구종 무관)
fit_mask = cls >= 0
...
LBL = ["nd&ballxFB","nd&ballxBK","nd&ballxOS","nd&strikexFB","nd&strikexBK","nd&strikexOS",
       "nd&otherxFB","nd&otherxBK","nd&otherxOS","middle","reverse"]
for c in range(NCLS):
    log(f"    class{c:2d} {LBL[c]:14s}: n={(cls==c).sum():,} ({(cls==c).mean()*100:.2f}%) succ={succ_by_cls[c]*100:.3f}%")

log("11-class softmax CatBoost training (full data, labeled rows only)...")
fit_idx = np.where(fit_mask)[0]
n_es = int(len(fit_idx) * 0.92)
tr_i, es_i = fit_idx[:n_es], fit_idx[n_es:]
CAT_PARAMS = dict(iterations=1000, learning_rate=0.05, depth=6, l2_leaf_reg=5.0, verbose=False,
                  random_seed=42, loss_function="MultiClass", classes_count=NCLS,
                  early_stopping_rounds=40)


class ProgressCallback:
    """CatBoost 내부 verbose 출력이 이 환경(백그라운드 캡처)에서 안 잡히는 문제 회피용.
    Python 레벨 print(flush=True)는 지금까지 항상 정상 캡처됐으므로 이걸로 대체한다."""
    ...
    def after_iteration(self, info):
        ...
        if it % self.period == 0 or it < 5:
            el = time.time() - self.t0
            eta = el / (it + 1) * (1000 - it - 1)
            log(f"    iter {it:4d}  valid_loss={loss:.6f}  best={self.best:.6f}@{self.best_iter}  "
                f"경과={el/60:.1f}분  잔여추정={eta/60:.1f}분")
        return True
...
mc_model.fit(X.iloc[tr_i], cls[tr_i], sample_weight=w[tr_i],
             eval_set=(X.iloc[es_i], cls[es_i]), callbacks=[ProgressCallback(period=10)])
