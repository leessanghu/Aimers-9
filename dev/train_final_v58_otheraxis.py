"""v58 = v50 + other축("기타" 범주, success+reverse+middle 합=0) 공유트리.
포렌식 발견: success+reverse+middle 합이 항상 1이 아님(합=0 13.17%="기타" 미모델링
범주, 합=2 3.41%="중증"). "기타"는 대응하는 원본 피처가 아예 없어 middle(122위/2회)
보다도 구조적으로 미활용. head1=(1-lab_other), 추론시엔 head0(y)만 사용.
...
Ymat = np.column_stack([y.astype(np.float64), head_other])

log("other축 공유트리 CatBoost 학습 (전체데이터)...")
...
ts = time.time()
otheraxis_model = CatBoostRegressor(**CAT_PARAMS)
otheraxis_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                    eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={otheraxis_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(otheraxis_model)

common = dict(v50)
common["otheraxis_model"] = otheraxis_model
common["otheraxis_weight"] = OTHERAXIS_WEIGHT
...
common["base_weight"] = 1.0 - existing - OTHERAXIS_WEIGHT
log(f"weights: base={common['base_weight']:.3f} otheraxis={OTHERAXIS_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v58.pkl")
