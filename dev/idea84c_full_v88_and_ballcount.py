"""1) 진짜 v88(risk보정+level_shift까지 적용된 최종출력) 기준으로 2스트라이크 슬로프 재검증.
2) 3볼(반대 압박) 카운트도 같은 방식으로 확인.
fold A, fold C 둘 다."""
...
df['is_3b'] = (df['balls_before'] == 3).astype(int)
...
def build_v88_final(p):
    """p='A'면 진짜 v88(risk보정+level_shift 전부 적용), p='C'면 8헤드 재정규화(risk/shift 없음 - 캐시부재)."""
    H = dict(
    ...
    if p == 'A':
        ...
        final = raw - float(v88['risk_alpha']) * (cut - float(v88['risk_center'])) + float(v88['level_shift'])
...
def slope_table(train_upto, cond_col, is_flag_col):
    train = df[df.season <= train_upto]
    g = train.groupby(['pitcher_id', is_flag_col])['control_success'].agg(['sum', 'count']).unstack(fill_value=0)
    ...
    return n1, (rate1 - rate0)


def run_fold(p, train_upto, val_year, flag_col, label):
    va = season == val_year
    yv = y[va]
    ...
    n1, gap = slope_table(train_upto, None, flag_col)
    va_idx = df.index[va]
    pid_va = df.loc[va_idx, 'pitcher_id'].to_numpy()
    flag_va = df.loc[va_idx, flag_col].to_numpy()
    n1_va = n1.reindex(pid_va).fillna(0).to_numpy(np.float64)
    raw_gap = np.nan_to_num(gap.reindex(pid_va).to_numpy(np.float64), nan=0.0)

    H1 = mth <= 6; H2 = ~H1
    print(f'--- {label} | fold {p} (train<={train_upto} -> val {val_year}), v88_final(risk+shift 적용={p=="A"}) ---')
    for K in [200, 880, 1500, 2500, 4000]:
        shrunk = raw_gap * (n1_va / (n1_va + K))
        applied = np.where(flag_va == 1, shrunk, 0.0)
        gains = []
...
print('########## 2스트라이크 (기존 검증, v88_final로 재확인) ##########')
run_fold('A', 2023, 2024, 'is_2k', '2스트라이크')
run_fold('C', 2021, 2022, 'is_2k', '2스트라이크')

print('########## 3볼 (신규, 반대 압박) ##########')
run_fold('A', 2023, 2024, 'is_3b', '3볼')
run_fold('C', 2021, 2022, 'is_3b', '3볼')
