"""v60 = v50에서 midaxis(0.10)+other(0.10)를 3-head 통합 공유트리(0.20)로 교체.
...
MIDOTHER_WEIGHT = 0.20
TRAIN_MONTH_LO, TRAIN_MONTH_HI = 3, 7  # 학습행 필터
...
log(f"학습행 필터: game_month {TRAIN_MONTH_LO}~{TRAIN_MONTH_HI} 만 사용 (검증셋 3-7월과 매칭)...")
month_mask = (df["game_month"] >= TRAIN_MONTH_LO) & (df["game_month"] <= TRAIN_MONTH_HI)
log(f"  전체 {len(df):,}행 중 3-7월 {month_mask.sum():,}행 ({month_mask.mean()*100:.1f}%) 사용")

log("투구단위 reverse/middle 라벨 복원 + other(합=0) 파생...")
...
Ymat = np.column_stack([y.astype(np.float64), head_mid, head_other])
log(f"  3-head 구성: y / 1-middle(율 {np.nanmean(lab_middle)*100:.2f}%) / 1-other(율 {np.nanmean(lab_other)*100:.2f}%)")

# 학습행을 3-7월로 하드필터 (ES홀드아웃도 이 부분집합 내에서 시행)
keep = month_mask.to_numpy() if hasattr(month_mask, "to_numpy") else month_mask
X = X.loc[keep].reset_index(drop=True)
Ymat = Ymat[keep]
w = w[keep]
log(f"  필터 후 학습행 {len(X):,}개")

log("middle+other 3-head 통합 공유트리 CatBoost 학습 (전체데이터)...")
...
ts = time.time()
midother_model = CatBoostRegressor(**CAT_PARAMS)
midother_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                    eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={midother_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(midother_model)

common = dict(v50)
common["midother_model"] = midother_model
common["midother_weight"] = MIDOTHER_WEIGHT
...
existing = sum(common.get(k, 0.0) for k in
              ["hurdle_weight", "mix_weight", "denoise_weight", "multi_weight", "multires_weight",
               "ordinal_weight"])
common["base_weight"] = 1.0 - existing - MIDOTHER_WEIGHT
log(f"weights: base={common['base_weight']:.3f} midother={MIDOTHER_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v60b.pkl")
