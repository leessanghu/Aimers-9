"""v25 = v23(team_te expanding) + 최근시즌 가중 학습(half-life=2시즌). 피처는 v15와 동일(91개).

발견(phase59): 시즌별 성공률이 계속 하락하는데(2019 0.565 -> 2024 0.486) 하락폭 자체가
매년 불규칙해서(21->22 -0.004, 22->23 -0.029, 23->24 -0.014), 가까운 과거 held-out으로
'다음 시즌 편향'을 예측하는 사후보정(상수 오프셋/로짓시프트/isotonic)은 전부 실패했다
(부호까지 반대로 나옴). season을 원시값 대신 상수이동으로 재인코딩하는 것도 트리 분할이
이동불변이라 완전 무의미했다(bit단위로 동일 결과).

반면 사후보정이 아니라 학습 과정 자체에서 최근 시즌 비중을 높이는 sample_weight
(half_life=2, weight=0.5**((max_season-season)/2))는 미래 편향을 '예측'하려 들지 않고
그냥 최근 데이터를 더 신뢰하게만 만들어서 이 함정을 피한다.

phase59 (2023->2024 폴드, team_te=expanding, HGB+CatBoost 단일시드):
  baseline(가중치 없음) = 839.3
  recency_weighted(half-life=2) = 861.9  (+22.6, 사후보정 없이 base 자체가 개선)

--- 이하 v23 설명 ---
v23 = v15(976.099 실증, 최고점)와 피처 100% 동일, team_te만 shuffled KFold -> expanding으로 교체.

features.py의 team_te가 shuffled KFold OOF라 seed가 바뀌면 완전히 다른 fold 조합으로
팀 인코딩이 계산되고, 이게 시간 구조와 안 맞아 학습 전체에 seed발 노이즈를 섞는다는 가설을
세워서(v21 seed=10 실측 +1.64 발견 이후) phase55에서 검증했다.

phase55 (2023->2024 폴드, seed 4개 평균, HGB+CatBoost 단일시드 50:50):
  shuffled_kfold(v15): 812.9/818.2/828.1/810.4 -> 평균 817.4, 표준편차 7.9
  expanding(v23):      839.3/831.4/844.7/847.0 -> 평균 840.6, 표준편차 6.7
  -> 전 seed에서 expanding 승리, 평균 +23.2, 표준편차도 감소.

같은 phase55에서 HGB max_leaf_nodes/CatBoost depth 스윕도 했는데 지금 값(31/6)이 최선이라
그대로 두고, 스태킹(가중치탐색/로지스틱)도 고정 0.5:0.5보다 전부 나빠서 폐기하고 그대로 둔다.
피처는 v15와 완전히 동일(91개) — team_te 계산 방식 하나만 바꾼 단일 변수 실험이다.
...
def main():
    ...
    print(f"\n교차항 {C.shape[1]}개 + 작년 {X_ly.shape[1]}개 -> 최종 피처 수={X.shape[1]} (v14 84 -> 91)", flush=True)

    print("\n[4c] 최근시즌 가중치 (half-life=2, phase59 recency_weighted)...", flush=True)
    ...
    for tag, w_hgb, w_cat in [("v25", 0.5, 0.5)]:
