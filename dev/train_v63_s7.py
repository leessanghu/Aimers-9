"""v63 = v50 + count_resid축(y - E[y|balls,strikes] 잔차). idea54 fold A 로컬Δ=-1.23
(음수)였으나 사용자 지시로 실측 직행. count prior는 전체 train에서 계산(K=500).
"""
import os
import sys
import time
...
log("count prior 계산 (train 전체, K=500) + 잔차 타겟 구성...")
...
K_C = 500.0
ctab["prior"] = (ctab["sum"] + K_C * y.mean()) / (ctab["count"] + K_C)
cprior = pd.Series(count_state).map(ctab["prior"]).to_numpy(np.float64)
head_countresid = y.astype(np.float64) - cprior
log(f"  count_prior range=[{cprior.min():.4f},{cprior.max():.4f}]  resid sd={head_countresid.std():.4f}")
Ymat = np.column_stack([y.astype(np.float64), head_countresid])

log("count_resid축 공유트리 CatBoost 학습 (전체데이터)...")
...
CAT_PARAMS = dict(iterations=1500, learning_rate=0.03, depth=6, l2_leaf_reg=5.0, verbose=0,
                  random_seed=7, loss_function="MultiRMSE", early_stopping_rounds=50)
ts = time.time()
countresid_model = CatBoostRegressor(**CAT_PARAMS)
countresid_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                     eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={countresid_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(countresid_model)

common = dict(v50)
common["countresid_model"] = countresid_model
common["countresid_weight"] = COUNTRESID_WEIGHT
...
common["base_weight"] = 1.0 - existing - COUNTRESID_WEIGHT
log(f"weights: base={common['base_weight']:.3f} countresid={COUNTRESID_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v63s7.pkl")
