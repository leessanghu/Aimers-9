"""v49 = v47 + 폼 나우캐스팅 공유트리 (아이디어A, fold A 로컬검증은 음성이었지만
실측 확인을 위해 프로덕션 학습). Rule.md §4 준수: head_form은 train 내부에서만
미래정보로 구성(공식 학습데이터), 추론시엔 head0(y)만 사용 -- test 행은 자기
컬럼만 참조.
"""
import os
import sys
...
log(f"head_form 구성 (향후 {WINDOW}투구 실제성공률, train 내부 미래정보)...")
pid = df["pitcher_id"].to_numpy()
rn = df["row_num"].to_numpy()
sub = pd.DataFrame({"pid": pid, "rn": rn, "y": y}, index=np.arange(len(df))).sort_values(["pid", "rn"])
grp = sub.groupby("pid")["y"]
...
head_form_sorted = (fut_sum + 10.0 * g) / (fut_cnt + 10.0)
head_form_sorted = head_form_sorted.fillna(g)
head_form = pd.Series(head_form_sorted.to_numpy(), index=sub.index).reindex(range(len(df))).to_numpy(np.float64)
log(f"  head_form: mean={head_form.mean():.4f} std={head_form.std():.4f}")

Ymat = np.column_stack([y.astype(np.float64), head_form])

log("폼 나우캐스팅 공유트리 CatBoost 학습 (전체데이터)...")
...
ts = time.time()
formcast_model = CatBoostRegressor(**CAT_PARAMS)
formcast_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                   eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={formcast_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(formcast_model)

common = dict(v47)
common["formcast_model"] = formcast_model
common["formcast_weight"] = FORMCAST_WEIGHT
...
common["base_weight"] = 1.0 - existing - FORMCAST_WEIGHT
log(f"weights: base={common['base_weight']:.3f} formcast={FORMCAST_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v49.pkl")
