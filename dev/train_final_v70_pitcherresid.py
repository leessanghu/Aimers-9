"""v70 = v50 + pitcher_resid축(y - h1). h1은 투수-시즌 LOO 성공률(multires v40과 동일
공식, K_PS=15). 기존 aux head(middle/other/ball/count잔차/미래50투구)는 전부 y를 다른
각도로 재정의했을 뿐이라 PCA진단상 PC1(투수실력, 분산 94.89%)로 수렴했다(idea57
확인: fold A/C corr(ability_here)가 count_resid_ref 대비 유의하게 낮음).
이 축은 방향이 반대: 타겟에서 투수실력을 명시적으로 빼서 남는 상황신호를 공유트리에
학습시킨다. head0(direct)만 추론에 사용.
"""
import os
import sys
import time
...
log("투수-시즌 LOO 실력(h1) 계산 (multires 공식과 동일, K=15) + 잔차 타겟 구성...")
...
head_pitcherresid = y.astype(np.float64) - h1
log(f"  h1 mean={h1.mean():.4f} std={h1.std():.4f}  resid std={head_pitcherresid.std():.4f}")
Ymat = np.column_stack([y.astype(np.float64), head_pitcherresid])

log("pitcher_resid축 공유트리 CatBoost 학습 (전체데이터)...")
...
CAT_PARAMS = dict(iterations=1500, learning_rate=0.03, depth=6, l2_leaf_reg=5.0, verbose=0,
                  random_seed=42, loss_function="MultiRMSE", early_stopping_rounds=50)
ts = time.time()
pitcherresid_model = CatBoostRegressor(**CAT_PARAMS)
pitcherresid_model.fit(X.iloc[tr_i], Ymat[tr_i], sample_weight=w[tr_i],
                       eval_set=(X.iloc[es_i], Ymat[es_i]))
log(f"  학습완료 best_iter={pitcherresid_model.best_iteration_} ({time.time()-ts:.0f}s)")
strip_rng(pitcherresid_model)

common = dict(v50)
common["pitcherresid_model"] = pitcherresid_model
common["pitcherresid_weight"] = PITCHERRESID_WEIGHT
...
common["base_weight"] = 1.0 - existing - PITCHERRESID_WEIGHT
log(f"weights: base={common['base_weight']:.3f} pitcherresid={PITCHERRESID_WEIGHT:.2f} (기존합={existing:.2f})")

out = os.path.join(OUT_DIR, "model_artifacts_v70.pkl")
